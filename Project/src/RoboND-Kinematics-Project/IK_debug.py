...
class IK_Service:
    ...
    def calculate_IK(self, req_pose):
	...
	# roll, pitch, yaw = end-effector orientation
        ...
        self.theta2 = pi/2 - alpha - (atan2(self.WC[2]-self.d1, sqrt(self.WC[0]**2+self.WC[1]**2)-self.a1)).evalf(subs = self.s)
        self.theta3 = pi/2 - (beta + 0.036)

        #4.) Calculate R_3_0
        R_3_0 = self.T_0_3[:3,:3].evalf(subs = {self.q1:self.theta1, self.q2:self.theta2, self.q3:self.theta3}).transpose()

        R_3_G = R_3_0 * R_0_G
            
        #5.) Compute q4-6
        self.theta4 = atan2(R_3_G[2,2],-R_3_G[0,2])
        self.theta5 = atan2(sqrt(R_3_G[2,2]**2+R_3_G[0,2]**2),R_3_G[1,2])
        self.theta6 = atan2(-R_3_G[1,1],R_3_G[1,0])

        return self.theta1, self.theta2, self.theta3, self.theta4, self.theta5, self.theta6

    def evaluate_IK(self):
        T_0_U = (self.T_0_3 * self.T_3_U).evalf(subs={self.q1:self.theta1, self.q2:self.theta2, self.q3:self.theta3, self.q4:self.theta4, self.q5:self.theta5, self.q6:self.theta6})
        EE = T_0_U[0:3,3]
        return self.WC, EE
...
def test_code(test_case):
    ...
    for x in xrange(0, len(req.poses)):
        theta1, theta2, theta3, theta4, theta5, theta6 = IK.calculate_IK(req.poses[x])
        ## 
        ####################################################################################
        ...
        your_wc, your_ee = IK.evaluate_IK()
    ########################################################################################
